...
def generate_image(
    ...
    try:
        ...
        final_prompt = f"{prompt}, {age_prompt}" if age_prompt else prompt
...
with gr.Blocks(title="ID Photo Generator") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            denoise = gr.Slider(label="Denoise Strength", minimum=0, maximum=1, step=0.05, value=0.5)
    ...
    generate_btn.click(
        fn=generate_image,
        inputs=[
            input_image, prompt, negative_prompt, steps, cfg, denoise,
            instantid_weight, seed, sampler_name, scheduler,
            # 추가된 입력들
